...
class IpfsSaveShare extends Contract {
     ...
     * @returns 信息
     */
    async UploadPublic(ctx, fp, ne, password) {
     ...
     * @param {String} password 加密密码，如果不加密则为任意值
     * @param {String} downloadPath 下载文件的路径
     * @returns 文件下载是否成功
     */
    async DownloadPublic(ctx, id, iv, password, downloadPath) {
        ...
        return downloadFileBlock(Kp, downloadPath);
    }
     ...
     * @returns 文件下载是否成功
     */
    async DownloadPrivate(ctx, id, downloadPath) {
        var usbkey = new USBKey();
        var blocks = [];
        // 根据id查询私有集合
        ...
        for (var i = 0; i < private_info.length; i++) {
            ...
            // 根据bid查询世界状态
            const asset = await ctx.stub.getState(bid);
            ...
            const downloadedBlock = fs.readFileSync(asset.hash);
            // 解密文件块
