...
class IpfsSaveShare extends Contract {
     ...
     * @returns 信息
     */
    async UploadPublic(ctx, fp, ne, Up, id) {
     ...
     * @param {*} fp 文件数据
     */
    async UploadPrivate(ctx, blocks, Bs, bids, id) {

        // 将blocks、Bs、bids从字符串状态恢复过来
        blocks = JSON.parse(blocks.toString());
        Bs = JSON.parse(Bs.toString());
    ...
    async DownloadPrivate(ctx, id) {
        ...
        for (var i = 0; i < private_info.length; i++) {
            ...
            // 根据哈希值下载加密文件块，将[原哈希，加密块]送入返回块
            ret.push([asset.hashedBlock, await downloadFileBlock(asset.hash)]);
        }
        return ret;
    }
